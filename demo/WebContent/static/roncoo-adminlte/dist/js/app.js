...
function _init() {
  ...
  $.AdminLTE.layout = {
    activate: function () {
      ...
      _this.fix();
      _this.fixSidebar();
      //20170611,对于页面采用了窗口调整大小的时候，重新修正页面尺寸
 ...
 */
(function ($) {
  // 打开tab页，此方法是点击左侧菜单的用例时，系统自动打开页面的事件处理功能
  //注意，目前该功能判断当前url是否打开过，如果打开过直接显示，否则重新加载一次
  $(".ajax-template").on("click","a[target='navTab']",function(){
    ...
    if (!$("#navTabs li[url='"+_href+"']").length) {
        ...
        $("#loading").show();
        //调用服务获取页面数据的处理过程，并且指定了请求返回的页面内容为html
        $.ajax({
          url:_href,
          dataType:"html",
          success:function(result){
