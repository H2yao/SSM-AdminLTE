...
function _init() {
   ...
   * 该方法必须有，否则系统的右测表栏将没有任何作用，点击都打不开设置的tab,同时切换迷你左侧菜单栏也不好使了
   * 
   */
  $.AdminLTE.controlSidebar = {
    //instantiate the object
    //该方法影响的是处理右侧边栏的设置问题，方法内部空实现，则打不开右设置侧边栏，但迷你侧边栏能打开
    activate: function () {
      //Get the object
      var _this = this;
      //Update options
 ...
 * 目前看，这个方法在index2.demo.html中没有走
 */
(function ($) {
  if(isDemo){
    log_cust("BOX REFRESH BUTTON执行了,但是立即返回，和没有走效果相同");
    return;
  }
...
(function ($) {
   if(isDemo){
    log_cust("EXPLICIT BOX CONTROLS执行了,但是立即返回，和没有走效果相同");
    return;
  }
...
(function ($) {
   if(isDemo){
    log_cust("TODO LIST CUSTOM PLUGIN执行了,但是立即返回，和没有走效果相同");
    return;
  }
 ...
 * 该方法执行了，而且右侧边栏的设置样式内容都由此处进行生成
 * 因为如果不走此代码那么index2.demo.html没有更换皮肤的功能了
 */
(function ($, AdminLTE) {
 if(isDemo){
    // log_cust("AdminLTE Demo Menu执行了,但是立即返回，和没有走效果相同");
    // return;
  }
